...
1. ✅ **PASS** Go server 起 WebTransport-over-HTTP/3 单 listener（`tether/poc/go-quic-wt/step1_server.go`）
...
PoC-2 进行中（2026-04-28 截至 step 1+2+5 PASS）。下面 P-XX 是 webtransport-go v0.10 的**实测行为事实**，每条都有 `tether/poc/go-quic-wt/step*.go` 复现。
